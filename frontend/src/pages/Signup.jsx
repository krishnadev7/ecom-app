...
const Signup = () => {
  ...
  const formik =  useFormik({
    ...
    onSubmit: async values => {
      values = await Object.assign(values, {profileImage: uploadedImage || ''})
      ...
      <div className='w-full max-w-sm bg-white m-auto flex flex-col items-center p-4'>
        ...
        <div className='w-20 h-20 overflow-hidden drop-shadow-md shadow-md rounded-full relative '>
          <label htmlFor='profileImage' className='cursor-pointer'>
            <img src={uploadedImage? uploadedImage : signUp} alt='' className='w-full ' />
            <div className='absolute bottom-0 h-1/3 bg-slate-500 bg-opacity-50 w-full h-full text-center '>
              <p className='text-sm p-1 text-white'>Upload</p>
            </div>
            <input type='file' id='profileImage' accept='image/*' className='hidden' onChange={onUpload}/>
          </label>
        </div>
